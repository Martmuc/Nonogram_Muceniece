//Pour faire fonctionner les prompts dans le terminal, il faut entrer cette commande dans le terminal VSC :
// npm install prompt-sync
// (Cela va ajouter le dossier node_modules et les fichiers package.json et package-lock.json,
//  pas besoin d'entrer cette ligne de commande s'ils sont déjà présents)
//Puis mettre cette ligne en haut du fichier:
...
var gridSolution = []
    for (let i = 0; i < gridHeight; i++){
        row = []
        for (let j = 0; j < gridWidth; j++){
            row.push(Math.floor(Math.random()*2)+1) // Remplissage pseudo-aléatoire de la Ligne i de la Grille Solution de Cases Remplies ou Marquées, avec équiprobabilité des cas.
        }
...
while (!finPartie()){
    printGrid(grid)
    while (inputLigne < 0 || inputColonne < 0){
        inputLigne = prompt("Sasir la Ligne de la case à modifier (de 0 à " + (gridHeight-1) + ") : ")
        if (!regexp.test(inputLigne) || inputLigne >= gridHeight) inputLigne = -1
        else inputLigne = parseInt(inputLigne)
        if (inputLigne >= 0){
            inputColonne = parseInt(prompt("Sasir la Colonne de la case à modifier (de 0 à " + (gridWidth-1) + "): "))
            if (!regexp.test(inputColonne) || inputColonne >= gridWidth) inputColonne = -1
            else inputColonne = parseInt(inputColonne)

        } 
    }
    grid[inputLigne][inputColonne] = (grid[inputLigne][inputColonne]+1)%3
    inputLigne = -1
    inputColonne = -1
}
printGrid(grid)
printGrid(gridSolution)
console.log('GG')