...
var columnsSolution = []
    for (let i = 0; i < gridWidth; i++){
        ...
        for (let j = 0; j < gridHeight; j++){
            column.push(testGrid[j][i])
        }
        columnsSolution.push(column)
    }

test("Liste des plages pour la ligne [1,1,2,2,2,1,2,1,1,1] = [2,1,3]", () => {
  expect(createHintList([1,1,2,2,2,1,2,1,1,1])).toStrictEqual([2,1,3]);
})

test("la ligne 0 de testGrid est : [2, 2, 1, 1, 2, 1, 2, 1, 1, 2]", () => {
  expect(testGrid[0]).toStrictEqual([2, 2, 1, 1, 2, 1, 2, 1, 1, 2]);
})

test("la ligne 3 de testGrid est : [2, 1, 1, 2, 1, 2, 1, 2, 2, 2]", () => {
  expect(testGrid[3]).toStrictEqual([2, 1, 1, 2, 1, 2, 1, 2, 2, 2]);
})

test("la ligne 9 de testGrid est : [2, 1, 1, 1, 1, 1, 2, 2, 1, 1]", () => {
  expect(testGrid[9]).toStrictEqual([2, 1, 1, 1, 1, 1, 2, 2, 1, 1]);
})

test("la colonne 0 de testGrid est : [2, 2, 1, 2, 2, 2, 2, 1, 2, 2]", () => {
  expect(columnsSolution[0]).toStrictEqual([2, 2, 1, 2, 2, 2, 2, 1, 2, 2]);
})

test("la colonne 2 de testGrid est : [1, 2, 1, 1, 2, 2, 2, 2, 1, 1]", () => {
  expect(columnsSolution[2]).toStrictEqual([1, 2, 1, 1, 2, 2, 2, 2, 1, 1]);
})

test("les plages de la ligne 0 de testGrid sont : [2, 1, 2]", () => {
  expect(createHintList(testGrid[0])).toStrictEqual([2, 1, 2]);
})

test("les plages de la ligne 5 de testGrid sont : [1, 1, 2, 1]", () => {
  expect(createHintList(testGrid[5])).toStrictEqual([1, 1, 2, 1]);
})

test("les plages de la colonne 8 de testGrid sont : [1, 1, 3]", () => {
  expect(createHintList(columnsSolution[8])).toStrictEqual([1, 1, 3]);
})